#!/usr/bin/env python3
"""
s06: Subagent — OpenAI SDK version

使用全新的 messages[] 创建子代理，实现上下文隔离：
  ...
  子代理工具: bash, read, write, edit, glob (无 task — 无递归)

与 s05 的变化：
  + task 工具 + spawn_subagent() 使用全新 messages[]
  + 安全限制: 每个子代理最多 30 轮
  + extract_text() 辅助函数
  子代理不能再创建子-子代理 (sub_tools 中无 task 工具)。
  主循环不变: task 通过 TOOL_HANDLERS 自动分发。

运行: python s06_subagent/code_openai.py
...
tracer = LLMTracer(name="s06_subagent", output_dir=str(WORKDIR / ".traces"))
...
SYSTEM = (
    ...
    "遇到复杂的子问题时，使用 task 工具创建子代理来处理。"
)

# s06: 子代理的系统提示词 — 无 task，无递归
SUB_SYSTEM = (
    f"你是一个编程助手，工作目录是 {WORKDIR}。"
...
def extract_text(content) -> str:
    ...
    # OpenAI: content 是字符串，tool_calls 是单独字段
    return "\n".join(
...
# ═══════════════════════════════════════════════════════════
#  NEW in s06: Subagent — 全新 messages[]，仅返回摘要
# ═══════════════════════════════════════════════════════════
...
def spawn_subagent(description: str) -> str:
    ...
    # Issue 5: 如果安全限制耗尽时仍在 tool_use，兜底处理
    result = extract_text(messages[-1].get("content"))
    if not result:
        # 最后一条是 tool_result，往前回溯找 assistant 文本
    ...
    return result  # 仅返回摘要，整个消息历史被丢弃

# 将 task 工具添加到主代理的工具列表
TOOLS.append({
    "type": "function",
    "function": {
        "name": "task",
...
if __name__ == "__main__":
    print("s06: Subagent — 创建子代理，全新上下文，仅返回摘要 (OpenAI SDK)")
